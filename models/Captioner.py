...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fine_tune(fine_tune=config['fine_tune_encoder'])
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        for t in range(max(decode_lengths)):
            ...
            attention_weighted_encoding, alpha = self.attention(encoder_out[:batch_size_t],
                                                                h[:batch_size_t])
            gate = 0 #self.sigmoid(self.f_beta(h[:batch_size_t]))  # gating scalar, (batch_size_t, encoder_dim)
            attention_weighted_encoding = gate * attention_weighted_encoding
            h, c = self.decode_step(
                torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1),
                (h[:batch_size_t], c[:batch_size_t]))  # (batch_size_t, decoder_dim)
            preds = self.fc(self.dropout(h))  # (batch_size_t, vocab_size)
            predictions[:batch_size_t, t, :] = preds    # (batch_size_t, max_caption_length, vocab_size)
            alphas[:batch_size_t, t, :] = alpha

        return predictions, encoded_captions, decode_lengths, alphas, sort_ind

class Decoder_NoTFNoAttNoGlove(nn.Module):
    """
    Decoder.
    """
    ...
    def __init__(self, config, vocab_size):
        ...
        self.dropout = nn.Dropout(p=self.dropout)
        self.decode_step = nn.LSTMCell(self.encoder_dim, self.decoder_dim, bias=True)  # decoding LSTMCell
        # self.decode_step2 = nn.LSTMCell(self.decoder_dim, self.decoder_dim, bias=True)  # decoding LSTMCell 2
        self.init_h = nn.Linear(self.encoder_dim, self.decoder_dim)  # linear layer to find initial hidden state of LSTMCell
        self.init_c = nn.Linear(self.encoder_dim, self.decoder_dim)  # linear layer to find initial cell state of LSTMCell
        self.sigmoid = nn.Sigmoid()
        self.fc = nn.Linear(self.decoder_dim, vocab_size)  # linear layer to find scores over vocabulary
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        for t in range(max(decode_lengths)):
            batch_size_t = sum([l > t for l in decode_lengths])
            h, c = self.decode_step(encoder_out[:batch_size_t].sum(dim=1), (h[:batch_size_t], c[:batch_size_t]))  # (batch_size_t, decoder_dim)
            # h2, c2 = self.decode_step2(h[:batch_size], (h2[:batch_size_t], c2[:batch_size_t]))  # (batch_size_t, decoder_dim)
            preds = self.fc(self.dropout(h))  # (batch_size_t, vocab_size)
            predictions[:batch_size_t, t, :] = preds    # (batch_size_t, max_caption_length, vocab_size)
